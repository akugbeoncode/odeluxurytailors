                          ...
                          <div class="link-details">
                            <div class="dropdown profile-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; right: 0px; transform: translate3d(23px, 27px, 0px);">
                                    ...
                                    <a class="dropdown-item" @click.prevent="attemptDeletingBrand(item.id, $event)"><i class="fa fa-trash m-r-5"></i> Delete</a>
                                </div>
                            </div>
                            <router-link
                              :to="{
  ...
  export default {
    ...
    data() {
      return {
        ...
        emptyMessage: 'Brand List is empty!'
      }
    },
    ...
    methods: {
      ...
      attemptDeletingBrand(brandID, event) {
        this.toggleParentMenuOptions(event);
        if (confirm(`Do you want to delete "${this.getCategoryByID(brandID).name}" brand?`)) {
          this.$store.dispatch(IS_LOADING, true)
          .then(() => {

            axios.defaults.headers.common['Authorization'] =  `${this.tokenType} ${this.token}`;
            axios.delete(`/managers/brands/${brandID}`, {
              headers: {
